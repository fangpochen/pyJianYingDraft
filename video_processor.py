...
USE_GPU_ACCEL = True # Attempt GPU acceleration for ffmpeg
...
# --- FFmpeg Helper Functions (Moved from jianying_utils.py) ---
...
def get_video_duration(video_path):
    ...
    try:
        ...
        # Log stderr for debugging, ensure encoding/errors='ignore' handles potential issues
        stderr_output = e.stderr.strip() if e.stderr else "No stderr output"
        logger.error(f"ffprobe 输出: {stderr_output}")
        raise RuntimeError(f"无法获取视频时长: {stderr_output}")
    except (KeyError, ValueError, json.JSONDecodeError) as e: # Added JSONDecodeError
        logger.error(f"解析 ffprobe 输出失败: {e}")
        logger.error(f"ffprobe stdout received: {result.stdout if 'result' in locals() else 'N/A'}") # Log stdout on parsing error
...
def split_video_ffmpeg(input_path, output_folder, num_segments=EXPECTED_SEGMENTS):
    ...
    if not os.path.exists(output_folder):
        try:
            ...
            return None # Cannot proceed without output folder

    try:
        duration = get_video_duration(input_path)
    except Exception as e:
        logger.error(f"无法获取视频时长，跳过切割 '{os.path.basename(input_path)}': {e}")
        return None # Indicate splitting failure

    if duration <= 0:
        logger.error(f"视频 '{os.path.basename(input_path)}' 时长为 0 或无效，无法切割。")
        return None

    segment_duration = duration / num_segments
    logger.info(f"准备将 '{base_name}{ext}' ({duration:.2f}s) 切割成 {num_segments} 段，每段约 {segment_duration:.2f}s")

    # Check if segment duration is too short (potential ffmpeg issue)
    if segment_duration < 0.1: # Example threshold, adjust if needed
    ...
    for i in range(num_segments):
        ...
        # Ensure segment doesn't try to read past the end (though -t should handle this)
        current_segment_duration = segment_duration
        #if start_time + segment_duration > duration:
        #    current_segment_duration = duration - start_time # Adjust last segment slightly if needed

        output_filename = f"{base_name}_part{i+1}{ext}"
        output_path = os.path.join(output_folder, output_filename)

        command = ['ffmpeg', '-y'] # Overwrite existing

        # --- Hardware Acceleration (Decoding) ---
        if USE_GPU_ACCEL:
            hwaccel_options = []
            if GPU_TYPE == 'nvidia': hwaccel_options = ['-hwaccel', 'cuda']
            elif GPU_TYPE == 'intel': hwaccel_options = ['-hwaccel', 'qsv', '-qsv_device', '/dev/dri/renderD128'] # Example for Linux, might need adjustment
            elif GPU_TYPE == 'amd': hwaccel_options = ['-hwaccel', 'dxva2'] # Primarily Windows

            if hwaccel_options:
                # Check if hwaccel is actually available before using it? (More complex)
                command = ['ffmpeg', '-y'] + hwaccel_options # Prepend hwaccel options before -i
                ...
                command = ['ffmpeg', '-y'] # Reset command if hwaccel not applicable
        else:
             command = ['ffmpeg', '-y']


        # --- Input, Time, and Codec Parameters ---
        command.extend(['-i', input_path])
        command.extend(['-ss', str(start_time)])
        command.extend(['-t', str(current_segment_duration)]) # Use potentially adjusted duration

        # --- Video/Audio Codecs (Encoding) ---
        codec_params = []
        ...
        elif USE_GPU_ACCEL and GPU_TYPE == 'intel':
            logger.debug(f"使用 Intel QSV 编码 (第 {i+1} 段)")
            # QSV params can vary, this is an example
            codec_params = ['-c:v', 'h264_qsv', '-preset', 'medium', '-b:v', '6M', '-maxrate', '10M']
        # Add elif for AMD AMF if needed
        else:
            if USE_GPU_ACCEL: logger.warning(f"GPU 类型 '{GPU_TYPE}' 不支持硬件编码或未配置，回退到 libx264 软件编码。")
        ...
        command.extend(['-c:a', 'aac', '-b:a', '128k']) # Audio codec

        # --- Other Flags ---
        # command.extend(['-threads', '0']) # Let ffmpeg decide threads, often better
        command.extend(['-movflags', '+faststart'])
        command.append(output_path)

        logger.info(f"  执行切割命令 (第 {i+1}/{num_segments} 段): {' '.join(command)}")
        try:
            # Use shell=False for security and correct argument parsing
            ...
            # Move logger calls to their own lines with proper indentation
            if process_result.stdout:
                 logger.debug(f"    ffmpeg stdout:\n{process_result.stdout.strip()}")
            if process_result.stderr:
                 logger.debug(f"    ffmpeg stderr:\n{process_result.stderr.strip()}") # Log stderr even on success for info
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0: # Check if file exists and is not empty
                ...
                break # Stop processing this video if one segment fails critically
        except FileNotFoundError:
            ...
            all_segments_successful = False
            break # Stop processing this video if one segment fails
        except Exception as e:
            logger.exception(f"切割第 {i+1} 段时发生未知错误")
            all_segments_successful = False
            break # Stop processing this video on unknown error

    if all_segments_successful and len(output_paths) == num_segments:
        logger.info(f"视频 '{base_name}{ext}' 成功切割成 {num_segments} 段。")
        return output_paths
    else:
        logger.error(f"视频 '{base_name}{ext}' 切割过程未成功生成所有期望的片段 ({len(output_paths)}/{num_segments})。")
        # Clean up any partially created segments for this video
        logger.info(f"  清理 '{base_name}{ext}' 可能产生的无效切割片段...")
        for p in output_paths: # Clean up segments that were created before failure
             try:
                 if os.path.exists(p): os.remove(p)
             except OSError as clean_err:
                 logger.warning(f"  清理失败片段时出错: {p} - {clean_err}")
        # Try cleaning up the one that might have failed if not in output_paths yet
        failed_segment_path = os.path.join(output_folder, f"{base_name}_part{len(output_paths)+1}{ext}")
        try:
             if os.path.exists(failed_segment_path): os.remove(failed_segment_path)
        except OSError as clean_err:
             logger.warning(f"  清理失败片段时出错: {failed_segment_path} - {clean_err}")
